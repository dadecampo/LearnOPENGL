...
public:
	...
	std::vector<Mesh>    meshes_face;
	std::vector<std::string>    textures_face;
	std::vector<std::string>    rendering_order;
	...
	void Draw(Shader& shader)
	{
		if (rendering_order.size() != 0) {
			...
			for (int i = 0; i < meshes.size(); i++) {
				auto meshCurr = meshes[i];
				int orderCurr = getTextureOrder(meshes[i].textures[0].path);
				for (int j = i + 1; j < meshes.size(); j++) {
					int orderPoss = getTextureOrder(meshes[j].textures[0].path);
...
private:
	...
	void loadModel(std::string const& path, std::string const& m_directory)
	{
		// read file via ASSIMP
		Assimp::Importer importer;
		const aiScene* scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_CalcTangentSpace);
	...
	int getTextureOrder(std::string textureName) {
		for (unsigned int i = 0; i < rendering_order.size(); i++) {
			if (textureName.find(rendering_order[i]) != std::string::npos) {
				return i;
			}
		}
		return -1;
	...
	int isFacialTexture(std::string textureName) {
		for (unsigned int i = 0; i < textures_face.size(); i++) {
			if (textureName.find(textures_face[i]) != std::string::npos) {
				return true;
			}
		}
		return false;
